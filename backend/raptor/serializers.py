...
def build_leg_geometry(
    ...
    coordinates = _shape_subpath(gtfs_trip_id, from_row, to_row, stop_times)
    if coordinates is None:
        lo, hi = sorted((from_row["stop_sequence"], to_row["stop_sequence"]))
        coordinates = _stop_to_stop_path(gtfs_trip_id, lo, hi)
...
def _shape_subpath(
    gtfs_trip_id: int, from_row: dict, to_row: dict, stop_times: List[dict]
) -> Optional[List]:
    """Tier 1 & 2: clip the trip's Shape, by distance-traveled if
    available, else by nearest shape point to each stop. Returns None if
    the trip has no usable Shape."""
    ...
    if from_row["stop__point"] is None or to_row["stop__point"] is None:
        return None
...
def _match_stops_to_shape(stop_times: List[dict], shape_points: List) -> Dict[int, int]:
    ...
    for row in stop_times:  # already ordered by stop_sequence
        point = row.get("stop__point")
        if point is None:
            continue
        idx = _nearest_point_index(shape_points, point, start_idx=search_start)
        mapping[row["stop_sequence"]] = idx
        search_start = idx
    return mapping


def _nearest_point_index(points: List, target, start_idx: int = 0) -> int:
    """Index of the point in `points[start_idx:]` closest to `target`
    (both (lon, lat)-like), never searching before `start_idx`. Squared
    distance is fine here since we're only comparing, not measuring, and
    everything's within one city-scale shape."""
