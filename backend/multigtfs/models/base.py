...
from multigtfs.compat import get_blank_value, write_text_rows, Manager, QuerySet
...
class Base(models.Model):
    ...
    def import_txt(cls, txt_file, feed, filter_func=None):
            ...
            def get_instance(value):
                if value.strip():
                    ...
                    if key1 not in cache:
                        ...
                        ).values_list(rel_name, "id")
                        cache[key1] = dict((str(x), i) for x, i in pairs)

                    # Create new?
                    if key2 not in cache[key1]:
                        kwargs = {related._rel_to_feed: feed, rel_name: value}
                        cache[key1][key2] = related.objects.create(**kwargs).id
        ...
        for unique_field in cls._unique_fields:
            assert unique_field in column_names, "{} not in {}".format(
                unique_field, column_names
            )
        ...
        for row in csv_reader:
            ...
            if len(new_objects) % batch_size == 0:  # pragma: no cover
                ...
                logger.info("Imported %d %s", count, cls._meta.verbose_name_plural)
                new_objects = []
        ...
        if extra_counts:
            extra_columns = feed.meta.setdefault("extra_columns", {}).setdefault(
                cls.__name__, []
            )
    ...
    def export_txt(cls, feed):
        ...
        extra_columns = feed.meta.get("extra_columns", {}).get(cls.__name__, [])
        ...
        for field_name in fields:
            if "__" in field_name:
                ...
                if model_name in model_to_field_name:
                    ...
                    cache[field_name] = dict((i, str(x)) for i, x in pairs)
        ...
        for item in objects.order_by(*sort_fields).iterator():
            ...
            for csv_name, field_name in column_map:
                ...
                if "__" in field_name:
                    ...
                    if isinstance(field, date):
                        formatted = field.strftime("%Y%m%d")
            ...
            for col in extra_columns:
                row.append(obj.extra_data.get(col, ""))
            rows.append(row)
            if len(rows) % batch_size == 0:  # pragma: no cover
                write_text_rows(csv_writer, rows)
                count += len(rows)
                logger.info("Exported %d %s", count, cls._meta.verbose_name_plural)
                rows = []
