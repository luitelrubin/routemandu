...
class Shape(Base):
    ...
    shape_id = models.CharField(
        max_length=255, db_index=True, help_text="Unique identifier for a shape."
    )
    geometry = models.LineStringField(
        null=True, blank=True, help_text="Geometry cache of ShapePoints"
    )
    ...
    def update_geometry(self, update_parent=True):
        ...
        points = self.points.order_by("sequence").values_list("point", flat=True)
        if len(points) > 1:
...
class ShapePoint(Base):
    ...
    point = models.PointField(help_text="WGS 84 latitude/longitude of shape point")
    sequence = models.IntegerField()
    traveled = models.FloatField(
        null=True, blank=True, help_text="Distance of point from start of shape"
    )
